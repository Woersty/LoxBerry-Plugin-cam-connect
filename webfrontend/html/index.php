...
$config_file = "$cam-connect5/config/system/general.cfg";
$cfg         = parse_ini_file($config_file, TRUE);
$lang        = $cfg["BASE"]["LANG"];

$plugin_config_file = "$cam-connect5/config/plugins/$cam-connect3/cam-connect.cfg";
...
$phrase_file = "$cam-connect5/templates/plugins/$cam-connect3/$lang/language.dat";
...
else 
{
  ...
  if ($watermark == 1) 
  {
  	// Create Cam Image Object
		...
		$stamp = imagecreatefrompng("$cam-connect5/webfrontend/html/plugins/$cam-connect3/watermark.png");
		$sx    = imagesx($stamp);
		$sy    = imagesy($stamp);
